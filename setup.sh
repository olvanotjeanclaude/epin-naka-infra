...
sudo apt-get update -y && sudo apt-get install -y gnupg software-properties-common curl wget gpg
...
# Verify key fingerprint (optional, non-blocking)
...
sudo gpg --dearmor --batch --yes -o /usr/share/keyrings/ansible-archive-keyring.gpg
